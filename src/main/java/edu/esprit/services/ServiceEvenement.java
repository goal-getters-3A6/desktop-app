...
public class ServiceEvenement implements IService<Evenement> {
    ...
    public void ajouter(Evenement eve ,String imagePath ) throws SQLException{
        ...
        String req = "SELECT COUNT(*) FROM evenement WHERE nom_eve = ? AND date_deve = ?";
        PreparedStatement ps = cnx.prepareStatement(req);
        ps.setString(1, eve.getNom_eve());
        ps.setDate(2, new Date(eve.getDated_eve().getTime()));

        ResultSet Rset = ps.executeQuery();
        if(Rset.next()) {
            int count = Rset.getInt(1);
            if(count > 0) {
                throw new SQLException("Un événement avec le même nom et la même date existe déjà.");
            }
        }
        req = "INSERT INTO evenement(nom_eve, date_deve, date_feve, nbr_max, adresse_eve, image_eve) VALUES (?, ?, ?, ?, ?, ?)";
